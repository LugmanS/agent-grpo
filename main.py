...
class JudgeParseError(Exception):
    """Raised when the judge response isn't valid JSON with a boolean `accept`."""
    pass

@retry(stop=stop_after_attempt(3))
async def final_answer_judge(full_question: str, pred: str, gold: str) -> bool:
    client = AsyncOpenAI(
        ...
        messages=[
            {
                "role": "system",
                "content": (
                    "You are given a question, the reference answer (labelled **Reference answer**), "
                    "and an answer generated by an AI assistant (labelled **AI answer**).\n\n"
                    "Your task is to decide whether the AI answer is correct and should be accepted. "
                    "You should accept the answer if it contains the relevant information from the reference answer. "
                    "You should not accept the answer if it is missing information relevant to the question, "
                    "or if it contradicts the reference answer.\n\n"
                    "ALWAYS output ONLY a valid JSON object with a single key named 'accept' that has a boolean value. "
    ...
    # Parse + validate; if invalid, raise so tenacity retries.
    try:
        obj = json.loads(raw_content)
    except Exception as e:
        raise JudgeParseError(f"Unparsable JSON: {raw_content!r}") from e

    if not isinstance(obj, dict) or "accept" not in obj or not isinstance(obj["accept"], bool):
        raise JudgeParseError(f"JSON must be an object with boolean 'accept'. Got: {obj!r}")
...
class RewardConfig:
    ...
    # tool reward params
    tool_base: float = 1.0
    tool_over_penalty_slope: float = 0.2
    tool_min_reward: float = -1.0
...
async def compute_reward(
    ...
    final_answer_judge: Callable[[str, str, str], bool],
    ...
    final_answer_judge(full_question, predicted_final, gold_final) -> bool
    """
    ...
    actual_calls = len(traj.steps)
    breakdown["tool_calls_required"] = float(required_calls)
    breakdown["tool_calls_actual"] = float(actual_calls)
    breakdown["tool_calls_reward"] = tool_reward_time_discounted(
        actual_calls=actual_calls,
        required_calls=required_calls,
        base=cfg.tool_base,
        over_penalty_slope=cfg.tool_over_penalty_slope,
        min_reward=cfg.tool_min_reward,
    )

    # (3) Final reward (LLM judge)
    is_final_correct = await final_answer_judge(item.full_question, traj.final_answer, item.final_answer)
    breakdown["final_correct"] = 1.0 if is_final_correct else 0.0
